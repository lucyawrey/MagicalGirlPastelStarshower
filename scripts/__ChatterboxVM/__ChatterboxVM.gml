...
function __ChatterboxVMInner(_instruction) {
	...
	if (is_string(_instruction.type)) {
		...
		if (
			!((_instruction.type == "if") || (_instruction.type == "else if"))
			...
			if ((_instruction.type == "option") && !leaving_option) {
				...
				if (_instruction.type == "option") {
					...
					var _optionString = _instruction
						.text
						.Evaluate(local_scope, filename, false);
				...
				switch (_instruction.type) {
					...
					case "forcewait":
						...
						if (__CHATTERBOX_DEBUG_VM) {
							__ChatterboxTrace(
								__ChatterboxGenerateIndent(_instruction.indent),
								"<<forcewait \"" + string(_system.__vmWaitName) + "\">>"
							);
						}
						break;
					...
					case "action":
						...
						var _direction_text = _instruction
							.text
							.Evaluate(local_scope, filename, true);
						...
						switch (CHATTERBOX_ACTION_MODE) {
							...
							case 1:
								_result = __ChatterboxEvaluate(
									local_scope,
									filename,
									__ChatterboxParseExpression(_direction_text, false),
									undefined,
									undefined
								);
								break;
							...
							case 2:
								_result = __ChatterboxEvaluate(
									local_scope,
									filename,
									__ChatterboxParseExpression(_direction_text, true),
									undefined,
									undefined
								);
								break;
