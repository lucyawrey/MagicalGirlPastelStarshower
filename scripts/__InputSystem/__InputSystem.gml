...
function __InputSystem() {
	...
	__InputTrace(
		"Welcome to Input by Juju Adams, Alynne Keith, and friends! This is version "
			+ INPUT_VERSION
			+ ", "
			+ INPUT_DATE
			+ " (GM version "
			+ string(GM_runtime_version)
			+ ")"
	);
						...
						if (INPUT_COLLECT_MODE == 1) {
							...
							if (!instance_exists(__InputUpdateController)) {
								...
								if (instance_exists(__InputUpdateController)) {
									if (GM_build_type == "run") {
										...
										__InputError(
											"__InputUpdateController has been deactivated\nPlease ensure that __InputUpdateController is never deactivated\nYou may need to use instance_activate_object(__InputUpdateController)"
										);
									} else {
										//Be nice when in production <:)
										__InputTrace(
											"Warning! __InputUpdateController has been deactivated. Please ensure that __InputUpdateController is never deactivated. You may need to use instance_activate_object(__InputUpdateController)"
										);
									}
								} else {
									...
									if (!_created) {
										...
										if (__restartTime == __time) {
											...
											if (GM_build_type == "run") {
												//Be nasty when running from the IDE >:(
												__InputError(
													"__InputUpdateController has been destroyed\nPlease ensure that __InputUpdateController is never destroyed"
												);
											} else {
												//Be nice when in production <:)
												__InputTrace(
													"Warning! __InputUpdateController has been destroyed. Please ensure that __InputUpdateController is never destroyed"
												);
											}
										}
									}
							...
							if (!__InputUpdateController.persistent) {
								if (GM_build_type == "run") {
									//Be nasty when running from the IDE >:(
									__InputError(
										"__InputUpdateController has been set as non-persistent\nPlease ensure that __InputUpdateController is always persistent"
									);
								} else {
									//Be nice when in production <:)
									__InputTrace(
										"Warning! __InputUpdateController has been set as non-persistent. Please ensure that __InputUpdateController is always persistent"
									);
							...
							if (
								__InputUpdateController.depth
								!= __INPUT_CONTROLLER_OBJECT_DEPTH
							) {
								if (GM_build_type == "run") {
									//Be nasty when running from the IDE >:(
									__InputError(
										"__InputUpdateController depth has been changed (expected ",
										__INPUT_CONTROLLER_OBJECT_DEPTH,
										", got ",
										__InputUpdateController.depth,
									...
									//Be nice when in production <:)
									__InputTrace(
										"Warning! __InputUpdateController depth has been changed (expected ",
										__INPUT_CONTROLLER_OBJECT_DEPTH,
										", got ",
										__InputUpdateController.depth,
										")\nPlease ensure that __InputUpdateController is never manually created and depth is not manually set"
