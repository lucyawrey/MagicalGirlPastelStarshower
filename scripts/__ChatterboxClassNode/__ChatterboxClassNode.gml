...
function __ChatterboxClassNode(
	...
	if (__CHATTERBOX_DEBUG_COMPILER) {
		__ChatterboxTrace("[", title, "]");
	}

	var _substring_array = __ChatterboxSplitBody(
		buffer,
		buffer_start,
		buffer_end,
		compile
	...
	static MarkVisited = function() {
		...
		if (_value == undefined) {
			...
			__ChatterboxVariableSetInternal(
				_long_name,
				_system.__variablesMap[? _long_name] + 1
			);
		}
	};
	...
	static __BuildLocalisation = function(_node_order, _node_dict, _buffer_batch) {
		...
		repeat (array_length(substring_array)) {
			...
			if (_substring_struct.line != _current_line_number) {
				if (_current_line.__Size() > 0) {
					array_push(_lines_array, _current_line);
				}
