...
function __InputGamepadIdentifySwitchType(
	_gamepadIndex,
	...
	if (_description == "Joy-Con") {
		...
		__InputTrace(
			"Warning! Using stop-gap identification for bad description \"Joy-Con\""
		);
		...
		if (_left) {
			...
			if (_right) {
				...
				__InputTrace(
					"Warning! Invalid left/right Joy-Con state, falling back on handheld"
				);
				return INPUT_GAMEPAD_TYPE_SWITCH;
			}
		}
