...
function InputDeviceEnumerate(_includeGenericDevice = false) {
	...
	if (!INPUT_BAN_GAMEPADS) {
		...
		if (
			...
			&& (
				INPUT_ON_MACOS
					|| ((!_system.__usingSteamworks) && INPUT_ON_WINDOWS)
					|| (_system.__usingSteamworks && INPUT_ON_LINUX)
			)
		) {
	...
	if (!INPUT_BAN_KBM) {
		array_push(_array, INPUT_KBM);
	}
	if (!INPUT_BAN_TOUCH) {
		array_push(_array, INPUT_TOUCH);
	}
	if (_includeGenericDevice) {
		array_push(_array, INPUT_GENERIC_DEVICE);
	}
