...
function __InputCompareVersions(_targetVersion, _actualVersion) {
	...
	static _funcSplit = function(_versionString) {
		...
		var _struct = {__major: 0, __minor: 0};

		//Break down the string whilst also filtering out non-numeric characters (and period .)
		string_foreach(_versionString, method({__array: _array}, function(_char) {
			var _ord = ord(_char);
		...
		if (_firstDotIndex < 0) {
			...
			if (_secondDotIndex < _firstDotIndex) {
				_struct.__minor = _funcSafeReal(
					string_concat_ext(_array, _firstDotIndex + 1)
				);
			} else {
				_struct.__minor = _funcSafeReal(
					string_concat_ext(
						_array,
						_firstDotIndex + 1,
						_secondDotIndex - _firstDotIndex - 1
					)
				);
			}
		}
	...
	if (_targetStruct.__major != _actualStruct.__major) {
		return false;
	}
	if (_targetStruct.__minor > _actualStruct.__minor) {
		return false;
	}
