...
function InputPressedMany(_verbIndexArray, _playerIndexArray = 0) {
	...
	if (is_array(_verbIndexArray)) {
		...
		repeat (array_length(_verbIndexArray)) {
			if (InputPressedMany(_verbIndexArray[_v], _playerIndexArray)) {
				return true;
			}
			++_v;
		}
		...
		if (is_array(_playerIndexArray)) {
			...
			repeat (array_length(_playerIndexArray)) {
				if (InputPressed(_verbIndexArray, _playerIndexArray[_p])) {
					return true;
				}
				++_p;
			}
			...
			repeat (INPUT_MAX_PLAYERS) {
				if (InputPressed(_verbIndexArray, _playerIndex)) {
					return true;
				}
				++_playerIndex;
			}
