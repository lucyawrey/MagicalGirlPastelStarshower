...
function __scribble_tick() {
	...
	repeat (max(__SCRIBBLE_GC_STEP_SIZE, ceil(sqrt(_size)))) {
		//Choose a step size that scales with the size of the cache, but doesn't get too big
		...
		if (_element.__last_drawn + __SCRIBBLE_CACHE_TIMEOUT < _frames) {
			if (__SCRIBBLE_VERBOSE_GC) {
				__scribble_trace(
					"\"",
					_element.__cache_name,
					"\" has timed out (",
					_frames,
					" > ",
					_element.__last_drawn,
					" + ",
					__SCRIBBLE_CACHE_TIMEOUT,
					")"
				);
			}
	...
	repeat (max(__SCRIBBLE_GC_STEP_SIZE, ceil(sqrt(_size)))) {
		...
		if ((_weak == undefined) || !weak_ref_alive(_weak)) {
			if (__SCRIBBLE_VERBOSE_GC) {
				__scribble_trace("Removing element \"", _name, "\" from cache");
			}
	...
	repeat (max(__SCRIBBLE_GC_STEP_SIZE, ceil(sqrt(_size)))) {
		...
		if ((_weak == undefined) || !weak_ref_alive(_weak)) {
			if (__SCRIBBLE_VERBOSE_GC) {
				__scribble_trace("Removing model \"", _name, "\" from cache");
			}
	...
	repeat (max(__SCRIBBLE_GC_STEP_SIZE, ceil(sqrt(_size)))) {
		...
		if (!weak_ref_alive(_weak)) {
			if (__SCRIBBLE_VERBOSE_GC) {
				__scribble_trace("Cleaning up glyph grid ", _gc_grid_ids[_grid_index]);
			}
