...
function ChatterboxVariableGet() {
	...
	if (!ds_map_exists(_system.__variablesMap, _name)) {
		if (CHATTERBOX_ERROR_UNSET_VARIABLE) {
			__ChatterboxError(
				"Chatterbox variable \"",
				_name,
				"\" cannot be read because it has not been set"
			);
		} else {
			__ChatterboxTrace(
				"Warning! Chatterbox variable \"",
				_name,
				"\" cannot be read because it has not been set. Returning default value ",
				__ChatterboxReadableValue(_default)
			);
		}
	...
	if (!is_numeric(_value) && !is_string(_value) && !is_bool(_value)) {
		...
		__ChatterboxError(
			"Variable \"" + _name + "\" has an unsupported datatype (" + _typeof + " = ",
