...
function __ChatterboxCompile(_in_substring_array, _root_instruction, _hash_prefix) {
	...
	array_push(
		_in_substring_array,
		new __ChatterboxClassBodySubstring(
			CHATTERBOX_END_OF_NODE_HOPBACK ? "hopback" : "stop",
			"command",
			infinity,
			0,
			undefined,
	...
	while (_s < _substring_count) {
		...
		if (__CHATTERBOX_DEBUG_COMPILER) {
			__ChatterboxTrace(
				"ln ",
				string_format(_line, 4, 0),
				" ",
				__ChatterboxGenerateIndent(_indent),
				_string
			);
		}
		...
		if (_type == "option") {
			...
			switch (_first_word) {
				...
				case "jump":
					...
					_instruction.destination = __ChatterboxCompilerRemoveWhitespace(
						_remainder,
						all
					);
					break;
				...
				case "if":
					if (_previous_instruction.line == _line) {
						_previous_instruction.condition = __ChatterboxParseExpression(
							_remainder,
							false
						);
						...
						_instruction.condition = __ChatterboxParseExpression(
							_remainder,
							false
						);
						_if_depth++;
				...
				case "elif":
				case "else if":
					if (CHATTERBOX_ERROR_NONSTANDARD_SYNTAX) {
						__ChatterboxError(
							"<<",
							_first_word,
				...
				case "forcewait":
					...
					_instruction.waitName = __ChatterboxCompilerRemoveWhitespace(
						_remainder,
						all
					);
					break;
				...
				case "stop":
					...
					if (_remainder != "") {
						__ChatterboxError(
							"Cannot use arguments with <<hopback>>, <<jumpback>>, <<fastforward>>, <<fastmark>>, or <<stop>>\n\Action was \"<<",
							_string,
							">>\""
						);
					} else {
				...
				default:
					if ((_first_word == "random") && (_remainder == "option")) {
						...
						if (_q >= _substring_count - 1) {
							__ChatterboxError(
								"Cannot use <<random option>> at the end of a node"
							);
						} else if (_in_substring_array[_q].type != "option") {
							__ChatterboxError(
								"Must use <<random option>> immediately before an option (type=",
			...
			repeat (array_length(_line_instructions)) {
				if (
					(_previous_instruction.type == "content")
					|| (_previous_instruction.type == "option")
				) {
					...
					if (_is_line_hash) {
						...
						if (_instruction_text == undefined) {
							__ChatterboxTrace(
								"Warning! Cannot apply a line hash to a non-textual instruction"
							);
						} else if (_instruction_text.loc_hash != undefined) {
							__ChatterboxError(
								"Cannot apply more than one line hash to an instruction"
							);
						} else {
							_instruction_text.loc_hash =
								_hash_prefix
								+ string_delete(
									_string,
									1,
			...
			if (_count <= 0) {
				__ChatterboxTrace(
					"Warning! Line contained no content or options, metadata \"\#",
					_string,
					"\" cannot be applied"
				);
			}
