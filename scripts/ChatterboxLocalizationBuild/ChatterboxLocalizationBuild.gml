	...
	repeat (array_length(_csv_path_array)) {
		...
		repeat (array_length(_file_order)) {
			...
			buffer_write(
				_output_buffer,
				buffer_text,
				__ChatterboxEscapeForCSV(_filename)
			);
			buffer_write(_output_buffer, buffer_text, "\",,,,\n");
			...
			repeat (array_length(_node_order)) {
				...
				buffer_write(
					_output_buffer,
					buffer_text,
					__ChatterboxEscapeForCSV(_node_title)
				);
				buffer_write(_output_buffer, buffer_text, "\",,,\n");
				...
				repeat (array_length(_string_order)) {
					...
					var _new_hash =
						"#"
						+ string_copy(
							md5_string_unicode(_new_text),
							1,
							__CHATTERBOX_TEXT_HASH_LENGTH
						);
					...
					buffer_write(
						_output_buffer,
						buffer_text,
						__ChatterboxEscapeForCSV(_write_text)
					);
					buffer_write(_output_buffer, buffer_text, "\"\n");
