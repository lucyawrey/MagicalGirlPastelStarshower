...
function __InputClassPlayer(_playerIndex) constructor {
	...
	__kbmBindingArray = array_create_ext(_verbCount, function(_index) {
		...
		return (_verbDefinition != undefined)
			? variable_clone(_verbDefinition.__kbmBinding)
			: [];
	});
	...
	__gamepadBindingArray = array_create_ext(_verbCount, function(_index) {
		...
		var _verbDefinition = _verbDefinitionArray[_index];
		return (_verbDefinition != undefined)
			? variable_clone(_verbDefinition.__gamepadBinding)
			: [];
	});
	...
	__verbMetadataArray = array_create_ext(_verbCount, function(_index) {
		...
		var _verbDefinition = _verbDefinitionArray[_index];
		return (_verbDefinition != undefined)
			? variable_clone(_verbDefinition.__metadata)
			: undefined;
	});
	...
	__clusterMetadataArray = array_create_ext(_clusterCount, function(_index) {
		...
		return (_clusterDefinition != undefined)
			? variable_clone(_clusterDefinition.__metadata)
			: undefined;
	});
	...
	__thresholdMinArray = array_create(
		INPUT_THRESHOLD.__SIZE,
		INPUT_GAMEPAD_THUMBSTICK_MIN_THRESHOLD
	);
	__thresholdMaxArray = array_create(
		INPUT_THRESHOLD.__SIZE,
	...
	static __UpdateStatus = function() {
		...
		if (_connected) {
			if (
				...
				if (InputDeviceIsGamepad(__device)) {
					__lastConnectedGamepadType = InputDeviceGetGamepadType(__device);
				}
			} else {
	...
	static __UpdateClusterThresholds = function() {
		static _funcUpdateThresholdType = function(_alternateArray) {
			...
			repeat (array_length(_alternateArray)) {
				...
				if (_binding != undefined) {
					...
					_thresholdType |= ((_binding == gp_axislh) || (_binding == gp_axislv))
						? 1
						: (((_binding == gp_axisrh) || (_binding == gp_axisrv)) ? 2 : 0);
				}
		...
		repeat (_clusterCount) {
			...
			var _thresholdType = 0;
			_thresholdType |= _funcUpdateThresholdType(
				_gamepadBindingArray[_clusterDefinition.__verbUp]
			);
			_thresholdType |= _funcUpdateThresholdType(
				_gamepadBindingArray[_clusterDefinition.__verbRight]
			);
			_thresholdType |= _funcUpdateThresholdType(
				_gamepadBindingArray[_clusterDefinition.__verbDown]
			);
			_thresholdType |= _funcUpdateThresholdType(
				_gamepadBindingArray[_clusterDefinition.__verbLeft]
			);

			if (_thresholdType <= 1) {
				//Default to .LEFT if none of the bindings are thumbsticks
			...
			if (_d <= 0.0) {
				...
				if (_bias > 0) {
					...
					if (_bias >= 1) {
						...
						var _dirMod = (_direction % _divisor) / _divisor;
						_dirMod = clamp(
							(_dirMod - 0.5 * 0.999 * _bias) / (1 - 0.999 * _bias),
							0,
							1
						);
