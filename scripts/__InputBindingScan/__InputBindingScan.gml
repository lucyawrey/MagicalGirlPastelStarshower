function __InputBindingScan(_device, _ignoreStruct, _allowStruct) {
	...
	static _funcFilter = function(_input, _ignoreStruct, _allowStruct) {
		//Returns <false> if the binding failed to pass the filter
		if (variable_struct_exists(_ignoreStruct, string(_input))) {
			return false;
		}
		if (
			is_struct(_allowStruct)
			&& (!variable_struct_exists(_allowStruct, string(_input)))
		) {
			return false;
		}
		return true;
	...
	if (_device == INPUT_KBM) {
		if (!INPUT_BAN_KBM) {
			...
			if (
				(_binding != undefined)
				&& _funcFilter(_binding, _ignoreStruct, _allowStruct)
			) {
				return _binding;
			}
		}
