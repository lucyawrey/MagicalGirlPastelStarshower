...
function InputPlugInDefine(
	...
	if (!__InputCompareVersions(_targetInputVersion, INPUT_VERSION)) {
		__InputError(
			"Plug-in \"",
			_alias,
			"\" is incompatible with this version of Input (",
			INPUT_VERSION,
			")\nTarget Input version is ",
			_targetInputVersion
		);
	}

	var _plugInStruct = new __InputClassPlugIn(
		_alias,
		_author,
		_version,
		_targetInputVersion,
