...
function InputDeviceIsAvailable(_device) {
	...
	if ((_device >= 0) && INPUT_BAN_GAMEPADS) {
		return false;
	}
	if ((_device == INPUT_KBM) && INPUT_BAN_KBM) {
		return false;
	}
	if ((_device == INPUT_TOUCH) && INPUT_BAN_TOUCH) {
		return false;
	}
	if (_device == INPUT_GENERIC_DEVICE) {
		return true;
	}

	return !ds_map_exists(_deviceMap, _device);
}
