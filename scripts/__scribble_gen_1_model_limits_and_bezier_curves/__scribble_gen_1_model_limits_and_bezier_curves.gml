...
function __scribble_gen_1_model_limits_and_bezier_curves() {
	...
	var _model_max_width =
		_element.__wrap_max_width - (_element.__padding_l + _element.__padding_r);
	if (_model_max_width < 0) {
		_model_max_width = infinity;
	}

	var _model_max_height =
		_element.__wrap_max_height - (_element.__padding_t + _element.__padding_b);
	...
	var _bezier_do =
		(_element_bezier_array[0] != _element_bezier_array[4])
	...
	if (_bezier_do) {
		...
		repeat (SCRIBBLE_BEZIER_ACCURACY - 1) {
			...
			_y1 = _y2;
			_x2 =
				3.0 * _inv_t * _inv_t * _t * _bx2
				+ 3.0 * _inv_t * _t * _t * _bx3
				+ _t * _t * _t * _bx4;
			_y2 =
				3.0 * _inv_t * _inv_t * _t * _by2
				+ 3.0 * _inv_t * _t * _t * _by3
				+ _t * _t * _t * _by4;
	...
	if (is_string(_line_spacing_raw)) {
		...
		if (string_char_at(_line_spacing_raw, _length) == "%") {
			try {
				...
				var _line_spacing_multiply =
					real(string_copy(_line_spacing_raw, 1, _length - 1)) / 100;
			} catch (_error) {
