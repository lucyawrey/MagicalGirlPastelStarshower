...
function __InputClassVirtualButton() constructor {
	...
	static GetThreshold = function() {
		static _struct = {mini: undefined, maxi: undefined};
	...
	static RecordHistory = function(_state) {
		...
		if (__recordHistory != is_array(__historyArray)) {
			if (__recordHistory) {
				__historyArray = array_create(
					INPUT_VIRTUAL_BUTTON_HISTORY_FRAMES + 1,
					undefined
				);
				__historyCount = 0;
				...
				repeat (INPUT_VIRTUAL_BUTTON_HISTORY_FRAMES + 1) {
					__historyArray[@ _i] = {x: undefined, y: undefined};
	...
	static GetHistoryDirection = function(_frames = INPUT_VIRTUAL_BUTTON_HISTORY_FRAMES) {
		...
		if (_frames > INPUT_VIRTUAL_BUTTON_HISTORY_FRAMES) {
			__InputError(
				...
				_frames,
				") cannot exceed INPUT_VIRTUAL_BUTTON_HISTORY_FRAMES (",
	...
	static __CaptureTouchpoint = function(_device) {
		...
		if (_over) {
			...
			if (__recordHistory) {
				__HistoryPush(_touch_x, _touch_y);
			}
		...
		if (!__held) {
			...
			if (!_sustain) {
				...
				if (_length <= 0) {
					...
					var _thresholdFactor =
						clamp(
							(_length - __thresholdMin)
								/ (__thresholdMax - __thresholdMin),
							0,
							1
						) / _length;
