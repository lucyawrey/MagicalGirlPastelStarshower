...
function __InputGamepadDiscover(_gamepadStruct) {
	...
	with (_gamepadStruct) {
		...
		__InputTrace(
			...
			_device,
			", desc=\"",
			gamepad_get_description(_device),
			"\", GUID=\"",
			gamepad_get_guid(_device),
			"\", buttons = ",
			gamepad_button_count(_device),
			", axes = ",
			gamepad_axis_count(_device),
			", hats = ",
			gamepad_hat_count(_device)
		);
		...
		if (INPUT_ON_SWITCH) {
			...
			if (__type == INPUT_GAMEPAD_TYPE_SWITCH) {
				...
				if (__INPUT_SWITCH_JOYCON_HORIZONTAL_HOLDTYPE) {
					...
					if (__type == INPUT_GAMEPAD_TYPE_JOYCON_LEFT) {
						InputPlugInGamepadSetMapping(_device, gp_shoulderl, function(
							_device
						) {
							return gamepad_button_value(_device, 16);
						});
						InputPlugInGamepadSetMapping(_device, gp_shoulderr, function(
							_device
						) {
							return gamepad_button_value(_device, 17);
						});
					} else {
						InputPlugInGamepadSetMapping(_device, gp_shoulderl, function(
							_device
						) {
							return gamepad_button_value(_device, 18);
						});
						InputPlugInGamepadSetMapping(_device, gp_shoulderr, function(
							_device
						) {
							return gamepad_button_value(_device, 19);
						});
					}
				} else {
					...
					if (__type == INPUT_GAMEPAD_TYPE_JOYCON_LEFT) {
						InputPlugInGamepadSetMapping(_device, gp_shoulderl, function(
							...
							return gamepad_button_value(_device, 6);
						});
						InputPlugInGamepadSetMapping(_device, gp_shoulderr, function(
							_device
						) {
							return gamepad_button_value(_device, 8);
						});
					} else {
						InputPlugInGamepadSetMapping(_device, gp_shoulderl, function(
							_device
						) {
							return gamepad_button_value(_device, 7);
						});
						InputPlugInGamepadSetMapping(_device, gp_shoulderr, function(
							_device
						) {
							return gamepad_button_value(_device, 9);
						});
					}
				}
			}
			...
			InputPlugInGamepadSetMapping(_device, gp_touchpadbutton, function(_device) {
				return gamepad_button_value(_device, gp_select);
			});
			...
			__xinput =
				INPUT_ON_WINDOWS
				&& (_device < 4)
				&& (!__InputStringContains(__guid, "000000007801")); //"H" (HID)
			...
			if (__xinput) {
				...
				__InputTrace(
					"Identifying gamepad type using VID + PID \"",
					__vendor + __product,
					"\""
				);
				...
				if (__type == undefined) {
					__InputTrace(
						"Unsuccessful; identifying gamepad using description \"",
						__description,
						"\""
					);
