...
function ChatterboxVariableSetConstant(_name, _value) {
	...
	if (string_pos(" ", _name)) {
		__ChatterboxError(
			"Chatterbox constant names must not contain spaces (\"",
			_name,
			"\")"
		);
		exit;
	...
	if (ds_map_exists(_system.__constantsMap, _name) && !_system.__constantsMap[? _name]) {
		__ChatterboxError(
			"Trying to set Chatterbox constant $",
			_name,
			" but it has already been declared as a variable"
		);
	}

	if (!is_numeric(_value) && !is_string(_value) && !is_bool(_value)) {
		__ChatterboxError(
			"Chatterbox constant values must be a number, a string, or a boolean (constant = ",
			_name,
			", datatype = \"",
			typeof(_value),
			"\", value = ",
