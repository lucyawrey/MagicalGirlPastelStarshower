...
function __scribble_gen_4_build_words() {
	...
	if (_glyph_count > 0) {
		...
		repeat (_glyph_count - 1) {
			//Ensure we fully handle the last word by including the null terminator in this loop
			...
			switch (_glyph_bidi) {
				...
				case __SCRIBBLE_BIDI.SYMBOL:
					...
					if (
						(_word_bidi != __SCRIBBLE_BIDI.WHITESPACE)
						&& (_word_bidi != __SCRIBBLE_BIDI.ISOLATED)
					) {
						_glyph_bidi = _word_bidi;
				...
				case __SCRIBBLE_BIDI.ISOLATED_CJK:
					if (_glyph_prev_whitespace) {
						__SCRIBBLE_GEN_WORD_NEW;
						_glyph_prev_whitespace = false;
					} else if (_word_bidi == __SCRIBBLE_BIDI.SYMBOL) {
						// If the current word has a neutral direction, inherit the direction of the next L2R or R2L glyph
						// When (if) we find an L2R/R2L glyph then copy that glyph state back into the word itself
						_word_bidi = _glyph_bidi;
						_word_grid[# _word_count, __SCRIBBLE_GEN_WORD.__BIDI_RAW] =
							_glyph_bidi;
						_word_grid[# _word_count, __SCRIBBLE_GEN_WORD.__BIDI] =
							_glyph_bidi;
					} else {
						__SCRIBBLE_GEN_WORD_NEW;
					}
				...
				case __SCRIBBLE_BIDI.R2L_ARABIC:
					if (_glyph_prev_whitespace) {
						...
						if (_word_bidi >= __SCRIBBLE_BIDI.R2L) {
							...
							repeat (_i - _j) {
								_glyph_grid[# _j, __SCRIBBLE_GEN_GLYPH.__X] +=
									_word_width;
								_word_width -= _glyph_grid[#
									_j,
			...
			if (_word_bidi < __SCRIBBLE_BIDI.R2L) {
				//Any non-R2L text is laid out left-to-right
				...
				// __SCRIBBLE_BIDI.R2L or __SCRIBBLE_BIDI.R2L_ARABIC
				_word_width -= _glyph_grid[# _i, __SCRIBBLE_GEN_GLYPH.__SEPARATION];
				_glyph_grid[# _i, __SCRIBBLE_GEN_GLYPH.__X] += _word_width;
