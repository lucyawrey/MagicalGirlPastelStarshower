				...
				InputPlugInRegisterCallback(
					...
					function() {
						...
						if (__joinVerb == undefined) {
							InputPlugInError(
								"Join verb not defined. Please call InputPartySetParams()"
							);
						}
						...
						if (__fillEmpty) {
							...
							do {
								...
								repeat (INPUT_MAX_PLAYERS - 1) {
									if (
										InputPlayerIsConnected(_p)
										&& (!InputPlayerIsConnected(_p - 1))
									) {
										__InputTrace(
											"Moving player ",
											_p,
						...
						repeat (__maxPlayers) {
							if (!InputPlayerIsConnected(_p)) {
								if (
									is_callable(__abortCallback)
									&& (InputPlayerConnectedCount() <= 0)
								) {
									var _device = InputDeviceGetNewActivityOnVerb(
										__leaveVerb,
										_p
									);
								...
								if (__joinVerb == INPUT_PARTY_ANY_BUTTON) {
									var _device = __InputPartyDeviceGetNewActivity(
										__leaveVerb,
										_p
									);
								} else {
