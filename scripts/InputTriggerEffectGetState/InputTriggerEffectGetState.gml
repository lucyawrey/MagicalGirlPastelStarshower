...
function InputTriggerEffectGetState(_trigger, _playerIndex = 0) {
	...
	if ((_device < 0) || (!InputDeviceIsConnected(_device))) {
		return undefined;
	}
	...
	if (INPUT_ON_PS5) {
		return ps5_gamepad_get_trigger_effect_state(_device, _trigger);
	}

	var _effect = undefined;
	if (_trigger == gp_shoulderlb) {
		if (_struct.__interceptedLeft) {
			return INPUT_TRIGGER_STATE.EFFECT_INTERCEPTED;
		}
		_effect = _struct.__effectLeft;
	} else if (_trigger == gp_shoulderrb) {
		if (_struct.__interceptedRight) {
			return INPUT_TRIGGER_STATE.EFFECT_INTERCEPTED;
		}
