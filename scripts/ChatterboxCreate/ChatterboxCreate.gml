...
function ChatterboxCreate() {
	...
	var _filename = ((argument_count > 0) && (argument[0] != undefined))
		? argument[0]
		: _system.__defaultFile;
	var _singleton = ((argument_count > 1) && (argument[1] != undefined))
		? argument[1]
		: CHATTERBOX_DEFAULT_SINGLETON;
	var _local_scope = ((argument_count > 2) && (argument[2] != undefined))
		? argument[2]
		: id;
	...
	if (is_numeric(_local_scope) && (_local_scope < 100000)) {
		__ChatterboxError(
			"Local scope set to an invalid instance ID (was ",
			_local_scope,
			", must be >= 100000)"
		);
	}
...
function __ChatterboxClass(_filename, _singleton, _local_scope) constructor {
	...
	if (!ChatterboxIsLoaded(_filename)) {
		__ChatterboxError(
			"Could not create chatterbox because \"",
			_filename,
			"\" is not loaded"
		);
		return undefined;
	...
	static __HopPush = function(_next) {
		if (__CHATTERBOX_DEBUG_VM) {
			__ChatterboxTrace(
				"Pushing to hop stack: node = <",
				current_node,
				">, next instruction = <",
				json_stringify(_next),
				">, filename = \"",
				filename,
				"\""
			);
		...
		array_push(hopStack, {next: _next, node: current_node, filename: filename});
	};
	...
	static Select = function(_index) {
		if (!VerifyIsLoaded()) {
			__ChatterboxError(
				"Could not select option because \"",
				filename,
				"\" is not loaded"
			);
			return undefined;
		...
		if (stopped) {
			__ChatterboxTrace(
				"Warning! Could not select option because this chatterbox has been stopped"
			);
			return undefined;
		}
		...
		if ((_index < 0) || (_index >= array_length(option))) {
			__ChatterboxTrace(
				"Out of bounds option index (got ",
				_index,
				", maximum index for options is ",
	...
	static Continue = function(_name = "") {
		...
		if (stopped) {
			__ChatterboxTrace(
				"Warning! Could not continue because this chatterbox has been stopped"
			);
			return undefined;
		}
	...
	static __CurrentlyProcessing = function() {
		...
		repeat (array_length(_system.__vmInstanceStack)) {
			if (_system.__vmInstanceStack[_i] == self) {
				return true;
			}
			++_i;
		}
	...
	static FastForward = function() {
		if (!VerifyIsLoaded()) {
			__ChatterboxError(
				"Could not fast forward because \"",
				filename,
				"\" is not loaded"
			);
			return undefined;
		...
		if (GetOptionCount() > 0) {
			__ChatterboxTrace(
				"Error! Player is being prompted to make a choice, cannot fast forward"
			);
			return undefined;
		}
		...
		if (!file.loaded) {
			if (loaded) {
				__ChatterboxTrace(
					"Warning! \"",
					filename,
					"\" has been unloaded, an in-progress chatterbox has been invalidated"
				);
	...
	static __ChangeNode = function(_newNode, _markAsVisited, _type) {
		...
		if (is_undefined(_system.__nodeChangeCallback)) {
			...
			is_method(_system.__nodeChangeCallback)
			|| script_exists(_system.__nodeChangeCallback)
		) {
			_system.__nodeChangeCallback(
				(_oldNode != undefined) ? _oldNode.title : undefined,
