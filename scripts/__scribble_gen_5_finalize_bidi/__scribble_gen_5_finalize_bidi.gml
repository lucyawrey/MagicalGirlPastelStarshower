...
function __scribble_gen_5_finalize_bidi() {
	...
	repeat (_word_count) {
		...
		if (_bidi <= __SCRIBBLE_BIDI.SYMBOL) {
			...
			var _prev_bidi = (_i > 0)
				? _word_grid[# _i - 1, __SCRIBBLE_GEN_WORD.__BIDI]
				: __SCRIBBLE_BIDI.SYMBOL;
			var _next_bidi = (_i < _word_count - 1)
				? _word_grid[# _i + 1, __SCRIBBLE_GEN_WORD.__BIDI]
				: __SCRIBBLE_BIDI.SYMBOL;
			...
			if (_prev_bidi <= __SCRIBBLE_BIDI.SYMBOL) {
				_prev_bidi = _next_bidi;
			} //symbol or whitespace
			if (_next_bidi <= __SCRIBBLE_BIDI.SYMBOL) {
			...
			var _new_bidi = (
				(_prev_bidi == _overall_bidi) || (_next_bidi == _overall_bidi)
			)
				? _overall_bidi
				: _prev_bidi;

			// If we *still* can't decide on the direction, default to L2R
			if (_new_bidi <= __SCRIBBLE_BIDI.SYMBOL) {
				_new_bidi = __SCRIBBLE_BIDI.L2R;
			} //symbol or whitespace
