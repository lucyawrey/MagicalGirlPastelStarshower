...
function __InputRegisterUpdate() {
	__InputPlugInRegisterCallbackInternal(INPUT_PLUG_IN_CALLBACK.UPDATE, 0, function() {
		...
		//Handle window state
		if (INPUT_ON_DESKTOP && (!INPUT_ON_WEB)) {
			if (os_is_paused()) {
				//Lost focus
				__windowFocus = false;
				...
				if (__windowFocus) {
					if (__pointerBlockedByWindowDefocus) {
						...
						__pointerBlockedByWindowDefocus =
							__InputGetMouseOutput() != undefined;
					}
				} else if (
		...
		if (!INPUT_BLOCK_MOUSE_CHECKS) {
			...
			if (
				__pointerBlockedByUser
				|| (__pointerBlockedByWindowDefocus && (!INPUT_ON_MACOS))
				|| ((!INPUT_BAN_KBM) && InputDeviceGetRebinding(INPUT_KBM))
			) {
			...
			__pointerMoved =
				point_distance(
					__prevPointerDeviceX,
					__prevPointerDeviceY,
					__pointerDeviceX,
					__pointerDeviceY
		...
		if (__usingSteamworks) {
			...
			if (__InputSteamHandlesChanged()) {
				__InputTrace(
					"Steam handles changed, disconnecting all gamepads for reconnection"
				);
		...
		if ((!INPUT_BAN_GAMEPADS) && (_frame > INPUT_GAMEPADS_TICK_PREDELAY)) {
			if (
				INPUT_ON_ANDROID
				&& (
					__time - __androidEnumerationTime
			...
			var _deviceCountChange = max(
				0,
				gamepad_get_device_count() - array_length(_gamepadArray)
			);
				...
				if (_gamepad != undefined) {
					if (_connected) {
						if (
							...
							&& (
								_gamepad.__type
									!= __InputGamepadIdentifySwitchType(_device)
							)
						) {
						...
						if (
							_frame - _gamepad.__lastConnectedTime
