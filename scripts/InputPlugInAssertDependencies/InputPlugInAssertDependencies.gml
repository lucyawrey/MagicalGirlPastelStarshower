...
function InputPlugInAssertDependencies() {
	...
	repeat (argument_count div 2) {
		...
		if (!is_struct(_plugInStruct)) {
			__InputError(
				"Plug-in \"",
				argument[_i],
				"\" required as a dependency but it cannot be found\nPlease import this missing plug-in"
			);
			return false;
		}

		if (!__InputCompareVersions(argument[_i + 1], _plugInStruct.__version)) {
			__InputError(
				"Required version (",
				argument[_i + 1],
				") of plug-in \"",
