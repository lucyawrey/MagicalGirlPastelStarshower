	...
	static __BuildSubstrings = function(_string) {
		...
		repeat (buffer_get_size(_buffer)) {
			...
			if (_byte == 0x00) {
				...
				if (CHATTERBOX_ESCAPE_CONTENT) {
					_substring = __ChatterboxUnescapeString(_substring);
				}
				array_push(substrings, _substring);
			} else if (_byte == ord("{")) {
				//GameMaker will optimise this at compile time
	...
	static Evaluate = function(_local_scope, _filename, _delimitStrings) {
		...
		repeat (array_length(substrings)) {
			...
			if (is_struct(_value)) {
				...
				if (is_string(_value) && _delimitStrings) {
					_value = "\"" + _value + "\"";
				}
			}
