...
function __InputGamepadTransformSteam(_gamepadStruct) {
		...
		with (_gamepadStruct) {
			...
			if (INPUT_ON_LINUX) {
				if (__INPUT_DEBUG_STEAM_INPUT) {
					__InputTrace(
						"Running Steam Input Linux logic for gamepad index ",
						_gamepadIndex
					);
				}
				...
				repeat (__gamepadIndex + 1) {
					if (
						...
						if (__INPUT_DEBUG_STEAM_INPUT) {
							__InputTrace("Gamepad index ", _i, " is a virtual gamepad");
						}
						_gamepadHandleIndex++;
			...
			if (
				(INPUT_ON_MACOS && (__guid == "030000005e0400008e02000001000000"))
				|| (
					INPUT_ON_LINUX
						&& (__guid == "03000000de280000ff11000001000000")
						&& !(_system.__onSteamDeck && _system.__usingSteam)
				)
			) {
				...
				__InputTrace(
					"Ignoring gamepad ",
					__gamepadIndex,
					" as it is a Steam virtual controller"
				);
				__blocked = true;
				//Don't return! We still want to process Steam variables
			} else {
				__steamHandle = steam_input_get_controller_for_gamepad_index(
					_gamepadIndex
				);

				var _check = steam_input_get_gamepad_index_for_controller(__steamHandle);
				__InputTrace(
					"Checking gamepad index: original=",
					__gamepadIndex,
					", remapped=",
					_gamepadIndex,
					", derived from Steam handle=",
					_check
				);

				if (__INPUT_DEBUG_STEAM_INPUT) {
					__InputTrace(
						"Found Steam Input handle ",
						__steamHandle,
						" for gamepad index ",
						_gamepadIndex
					);
				...
				if (
					...
					if (__INPUT_DEBUG_STEAM_INPUT) {
						__InputTrace(
							"Steam Input invalid for gamepad index ",
							_gamepadIndex
						);
					}
				} else {
					__steamHandleIndex = steam_input_get_gamepad_index_for_controller(
						__steamHandle
					);
					if (__INPUT_DEBUG_STEAM_INPUT) {
						__InputTrace(
							"Found Steam Input index ",
							__steamHandleIndex,
							" for gamepad index ",
							_gamepadIndex
						);
					...
					if (!(is_numeric(_steamType) && (_steamType >= 0))) {
						__InputTrace(
							"Warning! Steam gamepad type ",
							_steamType,
							" invalid"
						);
						__blocked = true;
					...
					if (__type == undefined) {
						__InputTrace(
							"Warning! Steam-gamepad-type ",
							_steamType,
							" has no matching Input-gamepad-type"
						);
	...
	if ((__vendor != "de28") && ds_map_exists(_steamInputTypeIgnoreMap, __type)) {
		__InputTrace(
			"Warning! Input-gamepad-type ",
			__type,
			" is blacklisted by Steam Input"
		);
		__blocked = true;
		return;
	}
