...
function InputMousePressed(_binding = mb_left) {
	...
	if (INPUT_BLOCK_MOUSE_CHECKS || _system.__pointerBlocked) {
		return false;
	}
	...
	if (INPUT_ON_WINDOWS && _system.__tapClick) {
		...
		if (INPUT_TOUCH_EDGE_DEADZONE > 0) {
			...
			if (
				(_x < INPUT_TOUCH_EDGE_DEADZONE)
				|| (_x > (display_get_gui_width() - INPUT_TOUCH_EDGE_DEADZONE))
				|| (_y < INPUT_TOUCH_EDGE_DEADZONE)
				|| (_y > (display_get_gui_height() - INPUT_TOUCH_EDGE_DEADZONE))
			) {
				var _left = false;
			} else {
