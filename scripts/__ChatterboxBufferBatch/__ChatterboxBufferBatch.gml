...
function __ChatterboxBufferBatch() constructor {
	...
	static __GetString = function() {
		if (__destroyed) {
			__Error("Worker has been destroyed");
		}
	...
	static __CommandClass = function(
		_type,
		_nth,
		_position,
		_content,
	...
	static __Insert = function() {
		...
		array_push(
			__commands,
			new __CommandClass("insert", array_length(__commands), _position, _content)
		);
	};
	...
	static __Overwrite = function() {
		...
		array_push(
			__commands,
			new __CommandClass("overwrite", array_length(__commands), _position, _content)
		);
	};
	...
	static __Prefix = function() {
		...
		array_push(
			__commands,
			new __CommandClass("prefix", array_length(__commands), -infinity, _content)
		);
	};
	...
	static __Suffix = function() {
		...
		array_push(
			__commands,
			new __CommandClass("suffix", array_length(__commands), infinity, _content)
		);
	};
	...
	static __FromBuffer = function(_buffer) {
		...
		if (__inBuffer != undefined) {
			__Error("Input buffer already loaded");
		}
	...
	static __CopyFromBuffer = function(
		_buffer,
		_start = 0,
		_count = (buffer_get_size(_buffer) - _start)
	) {
		if (__destroyed) {
			__Error("Worker has been destroyed");
		}
