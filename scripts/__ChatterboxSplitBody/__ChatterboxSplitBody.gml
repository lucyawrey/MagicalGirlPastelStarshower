...
function __ChatterboxSplitBody(
	_source_buffer,
	_source_buffer_start,
	_source_buffer_end,
	_apply_string_replacement
) {
	static _system = __ChatterboxSystem();

	if (__CHATTERBOX_DEBUG_SPLITTER) {
		__ChatterboxTrace(
			"Splitting body of node \"",
			title,
			"\" in file \"",
			filename,
			"\", metadata = ",
	...
	if (_apply_string_replacement && (ds_list_size(_system.__findReplaceOldString) > 0)) {
		...
		repeat (ds_list_size(_system.__findReplaceOldString)) {
			_work_string = string_replace_all(
				_work_string,
		...
		var _buffer = buffer_create(
			string_byte_length(_work_string) + 1,
			buffer_fixed,
			1
		);
	...
	var _func_read_string = function(
		_substring_array,
		_buffer,
		_string_start,
		_string_end,
		...
		if (_string_start > _string_end) {
			if (_type == "text") {
				if (__CHATTERBOX_DEBUG_SPLITTER) {
					__ChatterboxTrace("Empty text string");
				}
				return;
			}
		...
		var _substring = new __ChatterboxClassBodySubstring(
			_text,
			_type,
			_line,
			_indent,
			_buffer_offset + _string_start,
			_buffer_offset + _string_end
		);
	...
	if (__CHATTERBOX_DEBUG_SPLITTER) {
		__ChatterboxTrace("Line = ", _line);
	}

	while (true) {
		//Read until we hit the terminating null
		var _byte = buffer_read(_buffer, buffer_u8);

		if ((_byte == 0x00) || (_byte == 0x0A) || (_byte == 0x0D)) {
			//  null, \n, or \r
			if (_in_action) {
				__ChatterboxError(
					"Unfinished action found in node \"",
					title,
					"\" in file \"",
					filename,
					"\""
			...
			if (__CHATTERBOX_DEBUG_SPLITTER) {
				__ChatterboxTrace("Found newline");
			}

			if (_in_comment) {
				//Do nothing!
				if (__CHATTERBOX_DEBUG_SPLITTER) {
					__ChatterboxTrace("In comment, not writing substring");
				}
			} else {
				_func_read_string(
					_substring_array,
			...
			if (
				(_byte == 0x0D)
				&& (buffer_peek(_buffer, buffer_tell(_buffer), buffer_u8) == 0x0A)
			) {
			...
			if (_byte == 0x20) {
				...
				_find_indent = false;
				if (__CHATTERBOX_DEBUG_SPLITTER) {
					__ChatterboxTrace("Found start of content, ident=", _indent);
				}
				...
				if (
					...
					while (buffer_peek(_buffer, buffer_tell(_buffer), buffer_u8) == 0x20) {
						buffer_seek(_buffer, buffer_seek_relative, 1);
					}
			...
			if (
				_byte == ord("#")
				&& (
					buffer_peek(_buffer, max(0, buffer_tell(_buffer) - 2), buffer_u8)
						!= ord("\\")
				)
			) {
				// # but without leading \
				if (__CHATTERBOX_DEBUG_SPLITTER) {
					__ChatterboxTrace("Found start of metdata");
				}
				...
				//   //
				if (__CHATTERBOX_DEBUG_SPLITTER) {
					__ChatterboxTrace("Found start of comment");
				}
				...
				(_byte == ord(__CHATTERBOX_ACTION_OPEN_DELIMITER))
				&& (
					buffer_peek(_buffer, buffer_tell(_buffer), buffer_u8)
						== ord(__CHATTERBOX_ACTION_OPEN_DELIMITER)
				)
			) {
				if (__CHATTERBOX_DEBUG_SPLITTER) {
					__ChatterboxTrace("Found start of action");
				}
				...
				_in_action
				&& (_byte == ord(__CHATTERBOX_ACTION_CLOSE_DELIMITER))
				&& (
					buffer_peek(_buffer, buffer_tell(_buffer), buffer_u8)
						== ord(__CHATTERBOX_ACTION_CLOSE_DELIMITER)
				)
			) {
				if (__CHATTERBOX_DEBUG_SPLITTER) {
					__ChatterboxTrace("Found end of action");
				}

				var _substring = _func_read_string(
					_substring_array,
					_buffer,
					_string_start,
					_string_end,
