		...
		if (!INPUT_BAN_HOTSWAP) {
			if (__hotswap) {
				...
				if (InputPlayerGetInactive()) {
					...
					if (!InputDeviceIsActive(InputPlayerGetDevice())) {
...
function __InputUpdateGamepadPresence() {
	...
	if (
		INPUT_ON_ANDROID
		&& (
			__time - __androidEnumerationTime > INPUT_ANDROID_GAMEPAD_ENUMERATION_INTERVAL
		)
	) {
	...
	repeat (array_length(_gamepadArray)) {
		...
		if (_gamepad != undefined) {
			if (_connected) {
				if (
					INPUT_ON_SWITCH
					&& (_gamepad.__type != __InputGamepadIdentifySwitchType(_device))
				) {
				...
				if (
					current_time - _gamepad.__lastConnectedTime
