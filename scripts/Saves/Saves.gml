function save_game() {
	...
	if (obj_game.state.shared_is_touched) {
		var _shared_json = json_stringify(obj_game.state.shared);
	...
	if (obj_game.state.secret_is_touched) {
		var _secret_json = json_stringify(obj_game.state.secret);
	...
	if (obj_game.state.save_slot_is_touched) {
		var _save_slot_json = json_stringify(obj_game.state.save_slot);
		var _number_string = string(obj_game.state.shared.active_save_slot_id);
	...
	if (obj_game.state.player_is_touched) {
		var _player_json = json_stringify(obj_game.state.player);
		var _number_string = string(obj_game.state.shared.active_player_id);
...
function load_game() {
	if (file_exists(global.shared_path)) {
		...
		obj_game.state.shared = json_parse(_shared_json);
	} else {
		touch_shared();
	}
	...
	if (file_exists(global.secret_path)) {
		...
		obj_game.state.secret = json_parse(_secret_json);
	} else {
		touch_secret();
	}

	var _slot_number_string = string(obj_game.state.shared.active_save_slot_id);
	...
	if (file_exists(_slot_filename)) {
		...
		obj_game.state.save_slot = json_parse(_save_slot_json);
	} else {
		touch_slot();
	}

	var _player_number_string = string(obj_game.state.shared.active_player_id);
	...
	if (file_exists(_player_filename)) {
		...
		obj_game.state.player = json_parse(_player_json);
	} else {
		touch_player();
	}
...
function switch_slot(_slot_number, _update_player = true) {
	...
	if (obj_game.state.shared.active_save_slot_id == _slot_number) {
		return;
	}
	save_game();
	obj_game.state.shared.active_save_slot_id = _slot_number;
	touch_shared();
	save_game();
	obj_game.state.save_slot = variable_clone(obj_game.initial_state.save_slot);
	load_game();
	if (_update_player && obj_game.state.save_slot.player_id != -1) {
...
function switch_player(_player_id) {
	...
	if (obj_game.state.shared.active_player_id == _player_id) {
		return;
	}
	save_game();
	obj_game.state.shared.active_player_id = _player_id;
	touch_shared();
	save_game();
...
function switch_player_new() {
	var _player_count = obj_game.state.shared.player_count;
...
function get(_name) {
	if (string_starts_with(_name, "player_")) {
		...
		if (array_contains(global.player_base_variables, _name)) {
			return struct_get(obj_game.state.player, _name);
		}
		return struct_get(obj_game.state.player.data, _name);
	} else if (string_starts_with(_name, "secret_")) {
		_name = string_delete(_name, 1, 7);
		return struct_get(obj_game.state.secret.data, _name);
	} else {
		if (array_contains(global.slot_base_variables, _name)) {
			return struct_get(obj_game.state.save_slot, _name);
		}
		return struct_get(obj_game.state.save_slot.data, _name);
	}
}
...
function load_chatterbox_variables_from_state() {
	...
	array_foreach(global.slot_base_variables, function(_name) {
		struct_set(import, _name, struct_get(obj_game.state.save_slot, _name));
	});
	array_foreach(global.player_base_variables, function(_name) {
		struct_set(import, "player_" + _name, struct_get(obj_game.state.player, _name));
	});
	struct_foreach(obj_game.state.secret.data, function(_name, _value) {
		struct_set(import, "secret_" + _name, _value);
	});
	struct_foreach(obj_game.state.save_slot.data, function(_name, _value) {
		struct_set(import, _name, _value);
	});
	struct_foreach(obj_game.state.player.data, function(_name, _value) {
		struct_set(import, "player_" + _name, _value);
	});
	struct_foreach(obj_game.state.player.visited_nodes, function(_name, _value) {
...
function on_chatterbox_variable_set(_name, _value) {
	...
	if (string_starts_with(_name, "visited(default:")) {
		...
		struct_set(obj_game.state.player.visited_nodes, _name, _value);
	} else if (string_starts_with(_name, "player_")) {
		touch_player();
		_name = string_delete(_name, 1, 7);
		if (array_contains(global.player_base_variables, _name)) {
			struct_set(obj_game.state.player, _name, _value);
		} else {
			struct_set(obj_game.state.player.data, _name, _value);
		}
	} else if (string_starts_with(_name, "secret_")) {
		touch_secret();
		_name = string_delete(_name, 1, 7);
		struct_set(obj_game.state.secret.data, _name, _value);
	} else {
		touch_slot();
		if (array_contains(global.slot_base_variables, _name)) {
			struct_set(obj_game.state.save_slot, _name, _value);
		} else {
			struct_set(obj_game.state.save_slot.data, _name, _value);
		}
	}
