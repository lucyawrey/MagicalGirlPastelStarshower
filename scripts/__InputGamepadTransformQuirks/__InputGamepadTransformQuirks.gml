		...
		switch (os_type) {
			...
			case os_macosx:
				if (
					...
					(__type == INPUT_GAMEPAD_TYPE_JOYCON_LEFT)
					|| (__type == INPUT_GAMEPAD_TYPE_JOYCON_RIGHT)
				) {
			...
			case os_linux:
				if ((_buttonCount == 144) && (_axisCount == 0)) {
					...
					&& (_buttonCount == 13)
					&& (_hatCount == 1)
					&& __InputStringContains(_description, "usb")
				) {
					...
					__InputStringMatches(
						__guid,
						"03000000050b00000619000011010000",
						"03000000050b0000e318000011010000",
						"03000000050b0000e518000011010000",
						"03000000050b00005819000011010000",
						"03000000050b0000181a000011010000",
						"03000000050b00001a1a000011010000",
						"03000000050b00001c1a000011010000"
					)
				) {
					__InputTrace("Blocking gamepad: ROG Mouse");
					__blocked = true;
			...
			case os_tvos:
				if (__type == INPUT_GAMEPAD_TYPE_SWITCH) {
					...
					InputPlugInGamepadSetMapping(__gamepadIndex, gp_face1, function(
						_device
					) {
						return gamepad_button_value(__gamepadIndex, gp_face2);
					});
					InputPlugInGamepadSetMapping(__gamepadIndex, gp_face2, function(
						_device
					) {
						return gamepad_button_value(__gamepadIndex, gp_face1);
					});
					InputPlugInGamepadSetMapping(__gamepadIndex, gp_face3, function(
						_device
					) {
						return gamepad_button_value(__gamepadIndex, gp_face4);
					});
					InputPlugInGamepadSetMapping(__gamepadIndex, gp_face4, function(
						_device
					) {
						return gamepad_button_value(__gamepadIndex, gp_face3);
					});
				} else if (
					(__type == INPUT_GAMEPAD_TYPE_JOYCON_LEFT)
					|| (__type == INPUT_GAMEPAD_TYPE_JOYCON_RIGHT)
