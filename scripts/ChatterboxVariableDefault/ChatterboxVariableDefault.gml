	...
	if (ds_map_exists(_system.__defaultVariablesMap, _name)) {
		...
		if (_system.__defaultVariablesMap[? _name] != _value) {
			if (CHATTERBOX_ERROR_REDECLARED_VARIABLE) {
				__ChatterboxError(
					"Trying to re-declare default value for Chatterbox variable $",
					_name,
					" (=",
				...
				__ChatterboxTrace(
					"Warning! Trying to re-declare default value for Chatterbox variable $",
					_name,
					" (=",
					__ChatterboxReadableValue(_value),
					")"
