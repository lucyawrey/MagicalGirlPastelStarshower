...
function InputReleasedMany(_verbIndexArray, _playerIndexArray = 0) {
	...
	if (is_array(_verbIndexArray)) {
		...
		repeat (array_length(_verbIndexArray)) {
			if (InputReleasedMany(_verbIndexArray[_v], _playerIndexArray)) {
				return true;
			}
			++_v;
		}
	} else {
		//not is_array(_verbIndexArray)
		if (is_array(_playerIndexArray)) {
			var _p = 0;
			repeat (array_length(_playerIndexArray)) {
				if (InputReleased(_verbIndexArray, _playerIndexArray[_p])) {
					return true;
				}
				++_p;
			}
			...
			repeat (INPUT_MAX_PLAYERS) {
				if (InputReleased(_verbIndexArray, _playerIndex)) {
					return true;
				}
				++_playerIndex;
			}
