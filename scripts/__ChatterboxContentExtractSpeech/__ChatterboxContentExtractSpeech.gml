...
function __ChatterboxContentExtractSpeech(_string, _default) {
	...
	if ((_bracket_end_pos > 0) && (_bracket_end_pos > _colon_pos)) {
		...
		if ((_bracket_start_pos > 0) && (_bracket_start_pos < _colon_pos)) {
			_string = string_delete(
				_string,
				1,
				_bracket_end_pos + string_length(CHATTERBOX_SPEAKER_DATA_END) - 1
			);
	...
	var _speech = string_delete(
		_string,
		1,
		_colon_pos + string_length(CHATTERBOX_SPEAKER_DELIMITER) - 1
	);
