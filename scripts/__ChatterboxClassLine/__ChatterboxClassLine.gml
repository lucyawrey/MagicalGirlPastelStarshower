		...
		switch (_substring.type) {
			...
			case "metadata":
				...
				if (__ChatterboxMetadataStringIsLineHash(_text)) {
					...
					array_push(
						__hash_array,
						string_delete(_text, 1, __CHATTERBOX_LINE_HASH_PREFIX_LENGTH)
					);
				}
				break;
		}
	...
	static __BuildLocalisation = function(_string_array, _buffer_batch) {
		if (array_length(__text_substring_array) > array_length(__hash_array)) {
			...
			repeat (array_length(__substring_array)) {
				_last_buffer_pos = max(
					_last_buffer_pos,
					__substring_array[_i].buffer_end + 1
				);
				++_i;
			}
			...
			repeat (array_length(__text_substring_array) - array_length(__hash_array)) {
				...
				var _hash = string_copy(
					md5_string_unicode(string(__ChatterboxXORShiftRandom())),
					1,
			...
			repeat (array_length(__hash_array) - array_length(__text_substring_array)) {
				...
				_buffer_batch.__Delete(
					_substring.buffer_start - 1,
					1 + _substring.buffer_end - (_substring.buffer_start - 1)
				);
				++_i;
			}
		}
		...
		repeat (array_length(__text_substring_array)) {
			array_push(
				_string_array,
				{line_id: __hash_array[_i], content: __text_substring_array[_i].text}
			);
