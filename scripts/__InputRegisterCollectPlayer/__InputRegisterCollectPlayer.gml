...
function __InputRegisterCollectPlayer() {
	__InputPlugInRegisterCallbackInternal(
		INPUT_PLUG_IN_CALLBACK.COLLECT_PLAYER,
		0,
		function(_playerIndex) {
			...
			with (_playerArray[_playerIndex]) {
				...
				if (
					...
					|| __blocked
					|| __ghost
					|| InputDeviceGetRebinding(_device)
					|| (!InputGameHasFocus())
					|| __InputRestartTimeout()
				) {
					array_map_ext(_valueRawArray, function() {
						return 0;
					});
					array_map_ext(_valueClampArray, function() {
						return 0;
					});
				} else {
					if (
						(_device == INPUT_TOUCH)
						|| (INPUT_MOUSE_CAN_USE_VIRTUAL_BUTTONS && (_device == INPUT_KBM))
					) {
						...
						repeat (INPUT_MAX_TOUCHPOINTS) {
							if (device_mouse_check_button_pressed(_i, mb_left)) {
								...
								repeat (array_length(_virtualButtonArray)) {
									if (_virtualButtonArray[_j].__CaptureTouchpoint(_i)) {
										break;
									}
									++_j;
								}
					...
					if (_device >= 0) {
							...
							repeat (array_length(_alternateArray)) {
								...
								if (_rawBinding != undefined) {
									...
									var _raw = max(
										...
										sign(_rawBinding)
											* _readArray[
												_absBinding - INPUT_GAMEPAD_BINDING_MIN
											](_device, _absBinding)
									);
									if (_raw > _valueRaw) {
										_valueRaw = _raw;
										...
										if (
											(_absBinding == gp_shoulderlb)
											|| (_absBinding == gp_shoulderrb)
										) {
											_valueClamp = clamp(
												(
													_raw
											...
											(_absBinding == gp_axislh)
											|| (_absBinding == gp_axislv)
										) {
											_valueClamp = clamp(
												(_raw - _minLeft) / (_maxLeft - _minLeft),
												0,
												1
											);
										} else if (
											(_absBinding == gp_axisrh)
											|| (_absBinding == gp_axisrv)
										) {
											_valueClamp = clamp(
												(_raw - _minRight)
													/ (_maxRight - _minRight),
						...
						if (INPUT_MOUSE_CAN_USE_VIRTUAL_BUTTONS) {
							...
							var _i = 0;
							repeat (array_length(_virtualButtonArray)) {
								_virtualButtonArray[_i].__Collect(
									_valueRawArray,
									_valueClampArray
								);
								++_i;
