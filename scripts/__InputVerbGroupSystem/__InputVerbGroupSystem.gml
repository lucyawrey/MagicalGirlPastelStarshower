...
function __InputVerbGroupSystem() {
	...
	with (_system) {
		...
		InputPlugInDefine(
			"InputTeam.VerbGroups",
			"Input Team",
			"1.0",
			"10.0",
			function() {
				InputPlugInRegisterCallback(
					INPUT_PLUG_IN_CALLBACK.COLLECT_PLAYER,
					undefined,
					function(_playerIndex) {
						static _verbGroupLookupArray =
							__InputVerbGroupSystem().__verbGroupLookupArray;
						static _verbGroupInactiveArray =
							__InputVerbGroupSystem().__verbGroupInactiveArray;
						...
						repeat (array_length(_verbGroupLookupArray)) {
							if (INPUT_VERB_GROUP_INACTIVE_ON_ANY) {
								...
								if (
									(_verbBitmask != 0)
									&& ((_verbBitmask & _inactiveBitmask) == _verbBitmask)
								) {
									InputPlugInVerbSet(_verbIndex, 0, 0, _playerIndex);
								}
							}
				...
				InputPlugInRegisterCallback(
					INPUT_PLUG_IN_CALLBACK.FIND_BINDING_COLLISIONS,
					undefined,
					function(
						_collisionArray,
						_forGamepad,
						...
						if (_sourceBitmask == 0x00) {
							return;
						}

						var _i = array_length(_collisionArray) - 1;
						repeat (array_length(_collisionArray)) {
							var _foundBitmask = _verbGroupLookupArray[
								_collisionArray[_i].verbIndex
							];
							...
							if (_foundBitmask != 0x00) {
								...
								if (!(_sourceBitmask & _foundBitmask)) {
