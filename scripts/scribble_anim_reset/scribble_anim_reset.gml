...
function scribble_anim_reset() {
	...
	with (_scribble_state) {
		if (!__shader_anim_default) {
			...
			_array[@ __SCRIBBLE_ANIM.__CYCLE_SATURATION] =
				SCRIBBLE_DEFAULT_CYCLE_SATURATION;
			_array[@ __SCRIBBLE_ANIM.__CYCLE_VALUE] = SCRIBBLE_DEFAULT_CYCLE_VALUE;
			_array[@ __SCRIBBLE_ANIM.__JITTER_MINIMUM] =
				SCRIBBLE_DEFAULT_JITTER_MIN_SCALE;
			_array[@ __SCRIBBLE_ANIM.__JITTER_MAXIMUM] =
				SCRIBBLE_DEFAULT_JITTER_MAX_SCALE;
		...
		if (!__shader_anim_desync) {
			__shader_anim_desync = !__shader_anim_disabled; //Only re-set uniforms when the animations aren't disabled
