...
function __ChatterboxParseExpression(_string, _alt_direction_syntax) {
	...
	repeat (buffer_get_size(_buffer)) {
		...
		switch (_state) {
			...
			case __CHATTERBOX_TOKEN.STRING: //Quote-delimited String
				...
				if ((_byte == 0) || ((_byte == 34) && (_last_byte != 92))) {
					//null "
					_change_state = false;
					...
					if (CHATTERBOX_ESCAPE_EXPRESSION_STRINGS) {
						_read = __ChatterboxUnescapeString(_read);
					}
			...
			case __CHATTERBOX_TOKEN.NUMBER: //Number
				...
				if (_state != _next_state) {
					...
					try {
						...
						__ChatterboxError(
							"Error whilst converting expression value to real\n \n(",
							_error,
							")"
						);
						return undefined;
		...
		if (_new || (_state != _next_state)) {
			_read_start = _b;
		}
		_state = _next_state;
		if (_state == __CHATTERBOX_TOKEN.NULL) {
			break;
		}
		_last_byte = _byte;
	...
	if (array_length(_tokens) < 1) {
		...
		if (!_alt_direction_syntax) {
			if (array_length(_tokens) > 1) {
				__ChatterboxError(
					"Expression could not be fully resolved into a single token (",
					_string,
					")"
				);
			} else {
	...
	while (_t < array_length(_source_array)) {
		...
		if (is_struct(_token)) {
			if ((_token.op == "(") || (_token.op == "func")) {
				...
				if (_depth == 1) {
					var _sub_array = __ChatterboxArrayCopyPart(
						_source_array,
					...
					array_delete(
						_source_array,
						_sub_expression_start,
						array_length(_sub_array)
					);
					__ChatterboxCompileExpression(_sub_array);

					_source_array[@ _sub_expression_start] = {
						op: "param",
						a: _sub_array[0],
					};
				...
				if (_depth == 0) {
					...
					if (_is_function) {
						if (array_length(_sub_array) > 0) {
							var _parameters = __ChatterboxArrayCopyPart(
								_source_array,
								_open,
								1 + _sub_expression_start - _open
							);
							array_delete(
								_source_array,
								_open,
