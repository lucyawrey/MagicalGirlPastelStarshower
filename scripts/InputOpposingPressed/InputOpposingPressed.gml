...
function InputOpposingPressed(
	_verbNeg,
	_verbPos,
	_playerIndex = 0,
	_mostRecent = INPUT_OPPOSING_DEFAULT_MOST_RECENT
) {
	...
	if (_mostRecent) {
		...
		if (!_pressedNeg && !_pressedPos) {
			return 0;
		}

		if (_pressedPos && (!InputCheck(_verbNeg, _playerIndex))) {
			return 1;
		}
		if (_pressedNeg && (!InputCheck(_verbNeg, _playerIndex))) {
			return -1;
		}
