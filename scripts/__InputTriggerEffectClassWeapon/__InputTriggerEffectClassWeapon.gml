...
function __InputTriggerEffectClassWeapon(_trigger, _start, _end, _strength) constructor {
	...
	__params[$ __INPUT_TRIGGER_EFFECT_KEY_END_POSITION] = clamp(
		_end * 10,
	...
	static __SteamGetState = function(_gamepad, _trigger) {
		...
		if (
			_trigger_value
			> min(9.9, __params[$ __INPUT_TRIGGER_EFFECT_KEY_END_POSITION] + 2) / 10
		) {
			return INPUT_TRIGGER_STATE.EFFECT_WEAPON_FIRED;
		} else if (
			_trigger_value
			>= __params[$ __INPUT_TRIGGER_EFFECT_KEY_START_POSITION] / 10
		) {
