	...
	with (_system) {
		...
		if (array_length(_newSteamHandles) != array_length(_oldSteamHandles)) {
			...
			if (__INPUT_DEBUG_STEAM_INPUT) {
				__InputTrace(
					"Steam handle array length mismatch (old=",
					array_length(_oldSteamHandles),
					", new=",
					array_length(_newSteamHandles),
					")"
				);
			}
			return true;
		...
		repeat (array_length(_newSteamHandles)) {
			if (_newSteamHandles[_i] != _oldSteamHandles[_i]) {
				...
				if (__INPUT_DEBUG_STEAM_INPUT) {
					__InputTrace(
						"Steam handle value changed (index=",
						_i,
						", old=",
						_oldSteamHandles[_i],
						", new=",
		...
		repeat (array_length(__gamepadArray)) {
			...
			if (
				is_struct(_gamepad)
				&& (_gamepad.__steamHandle != undefined)
				&& (
					_gamepad.__steamHandleIndex
						!= steam_input_get_gamepad_index_for_controller(
							_gamepad.__steamHandle
						)
				)
			) {
				...
				if (__INPUT_DEBUG_STEAM_INPUT) {
					__InputTrace(
						"Steam gamped index changed (index=",
						_i,
						", gamepad=",
						_gamepad.__gamepadIndex,
						", handle=",
						_gamepad.__steamHandle,
						", old handle index=",
						_gamepad.__steamHandleIndex,
