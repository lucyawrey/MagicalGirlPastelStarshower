...
function __scribble_font_add_from_info(
	_name,
	_texture_group,
	_texture_uvs,
	_font_info,
	_line_height = undefined,
	_is_krutidev,
	_from_bundle
) {
	...
	try {
		...
		if (!__scribble_texture_group_get_ready(_texture_group)) {
			//Uhoh, let's check to see if we've gotten valid texture dimensions
			if (
				(texture_get_width(_texture_index) == 1)
				...
				_texels_valid = false;
				__scribble_trace(
					"Font \"" + _name + "\" texture not ready, possibly a dynamic texture"
				);
			}
		}
		...
		var _material = __scribble_get_material(
			_name,
			_texture_index,
			_sdf ? __SCRIBBLE_RENDER_SDF : __SCRIBBLE_RENDER_RASTER,
			_sdf_pxrange,
			_sdf_thickness_offset,
			_font_data.__bilinear
		);
		ds_grid_set_region(
		...
		var _GM_scaling =
			_font_info.size
			/ _font_glyph_data_grid[# _space_index, SCRIBBLE_GLYPH.HEIGHT];
		if (_GM_scaling > 1) {
			__scribble_trace(
				"Warning! Font \"",
				_name,
