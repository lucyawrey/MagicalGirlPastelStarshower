...
function __InputClassGamepad(_gamepadIndex) constructor {
	...
	static __UpdatePrevValues = function() {
		if (INPUT_BAN_GAMEPADS || __blocked || (!gamepad_is_connected(__gamepadIndex))) {
		...
		__valueArray[@ 0] = __readArray[gp_axislh - INPUT_GAMEPAD_BINDING_MIN](
			__gamepadIndex,
			gp_axislh
		);
		__valueArray[@ 1] = __readArray[gp_axislv - INPUT_GAMEPAD_BINDING_MIN](
			__gamepadIndex,
			gp_axislv
		);
		__valueArray[@ 2] = __readArray[gp_axisrh - INPUT_GAMEPAD_BINDING_MIN](
			__gamepadIndex,
			gp_axisrh
		);
		__valueArray[@ 3] = __readArray[gp_axisrv - INPUT_GAMEPAD_BINDING_MIN](
			__gamepadIndex,
			gp_axisrv
		);
	};
