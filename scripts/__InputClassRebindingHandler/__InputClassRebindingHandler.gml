...
function __InputClassRebindingHandler(_device, _ignoreArray, _allowArray) constructor {
	...
	if (is_struct(_ignoreArray)) {
		...
		var _ignoreStruct = {};
	} else if (!is_array(_ignoreArray)) {
		__InputError(
			"Rebinding ignore data must be an array or `undefined` (was \"",
			typeof(_ignoreArray),
			"\")"
		);
	} else {
	...
	} else if (!is_array(_allowArray)) {
		__InputError(
			"Rebinding allow data must be an array or `undefined` (was \"",
			typeof(_allowArray),
			"\")"
		);
	} else {
	...
	static __Update = function() {
		...
		var _binding = __InputBindingScan(
			__device,
			__rebindingIgnoreStruct,
			__rebindingAllowStruct
		);
		if (__rebindingWait) {
			if (_binding == undefined) {
				__rebindingWait = false;
			}
		} else {
			if (_binding != undefined) {
