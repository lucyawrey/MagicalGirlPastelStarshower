...
function __scribble_gen_10_write_vbuffs() {
	...
	var _func_region_pop = function(
		...
		while (_region_end >= _region_bbox_start) {
			_region_bbox_end = min(
				_region_end,
				_word_grid[#
					_line_grid[# _line, __SCRIBBLE_GEN_LINE.__WORD_END],
	...
	repeat (__pages) {
		...
		repeat (_page_data.__glyph_count) {
			...
			if (_glyph_ord >= 0) {
				...
				(_glyph_ord == __SCRIBBLE_GLYPH_SURFACE)
				|| (_glyph_ord == __SCRIBBLE_GLYPH_TEXTURE)
			) {
