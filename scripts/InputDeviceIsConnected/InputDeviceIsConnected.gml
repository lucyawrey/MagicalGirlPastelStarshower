...
function InputDeviceIsConnected(_device) {
	...
	if (_device >= 0) {
		if (INPUT_BAN_GAMEPADS || (_device >= array_length(_gamepadArray))) {
			return false;
		}

		var _struct = _gamepadArray[_device];
		return (_struct != undefined) && (!_struct.__blocked);
	} else if (_device == INPUT_KBM) {
		return !INPUT_BAN_KBM;
	} else if (_device == INPUT_TOUCH) {
		return (!INPUT_BAN_TOUCH) || (INPUT_ALLOW_TOUCH_ON_DESKTOP && INPUT_ON_DESKTOP);
