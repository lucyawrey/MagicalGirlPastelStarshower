...
function __InputVibrateClassCurve(
	_strength,
	_curve,
	_pan,
	_duration,
	...
	static __Update = function() {
		...
		if (_count == 1) {
			...
			if (_left != -1) {
				__outputLeft = clamp(animcurve_channel_evaluate(_left, _t), 0, 1);
			}
			if (_right != -1) {
				__outputRight = clamp(animcurve_channel_evaluate(_right, _t), 0, 1);
			}
		}
