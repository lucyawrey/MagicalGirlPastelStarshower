...
function __InputInitializeSteam() {
	with (__InputSystem()) {
			...
			if (_deck_envar != "") {
				...
				if (ds_exists(_map, ds_type_map)) {
					...
					if (
						...
						&& __InputStringContains(_identifier, "AMD Custom GPU 0")
					) {
						__onSteamDeck = true;
					}
		...
		var _switchLabels = environment_get_variable(
		...
		if (INPUT_ON_LINUX) {
			...
			if (
				((_steamEnviron != "") && (_steamEnviron == "1") || __usingSteamworks)
				&& (_steamConfigs != "")
				&& (_steamConfigs == string_digits(_steamConfigs))
			) {
