...
function scribble_markdown_format(_string) {
	if (
		(SCRIBBLE_COMMAND_TAG_OPEN != ord("["))
		|| (SCRIBBLE_COMMAND_TAG_CLOSE != ord("]"))
		|| (SCRIBBLE_COMMAND_TAG_ARGUMENT != ord(","))
	) {
		__scribble_error(
			"scribble_markdown_format() is not supported with non-standard command tag open/close/argument delimiters\nPlease request this feature if you need it"
		);
		return _string;
	}

	static _func_delete_buffer = function(
		_buffer_a,
		_buffer_size,
		_delete_size,
		_pos = buffer_tell(_buffer_a) - 2
	) {
	...
	static _func_insert_buffer = function(
		_buffer_a,
		_buffer_size,
		_insert_string,
		_write_pos = buffer_tell(_buffer_a) - 2
	) {
		static _buffer_b = __scribble_initialize().__buffer_b;
	...
	static _func_delete_and_insert_buffer = function(
		_buffer_a,
		_buffer_size,
		_delete_size,
		_insert_string = "",
	...
	var _fallback_styles_struct = {
		body: {},
		header1: {bold: true, italic: true, scale: 1.6},
		header2: {bold: true, scale: 1.4},
		header3: {italic: true, scale: 1.2},
		quote: {color: #E7E7E7, italic: true, scale: 0.9, prefix: "  "},
		bold: {bold: true},
		italic: {italic: true},
		bold_italic: {bold: true, italic: true},
		bullet_sprite: scribble_fallback_bulletpoint,
		link: {bold: true, color: #DF9FFF},
	};

	var _markdown_styles_struct = __scribble_initialize()
		.__state
	...
	while (true) {
		...
		if ((_value == 10) || (_value == 13)) {
			...
			if (_indent) {
				_indent = false;
				_buffer_size += _func_insert_buffer(
					_buffer,
					_buffer_size,
					"[/indent]",
		...
		if (_newline) {
			if ((_value == ord(">")) && (_next_value == 0x20)) {
				...
				if (_indent) {
					...
					_indent = true;
					_buffer_size += _func_delete_and_insert_buffer(
						_buffer,
						_buffer_size,
						2,
						"[indent]"
				...
				while (true) {
					var _header_next_value = buffer_peek(
						_buffer,
						_header_peek,
						buffer_u8
					);
				...
				if (_header_level > 0) {
					...
					_buffer_size += _func_delete_buffer(
						_buffer,
						_buffer_size,
						_header_level + 1
					);
				...
				((_value == ord("-")) || (_value == ord("*"))) && (_next_value == 0x20)
			) {
				//Unordered list
				_new_style = "body";
				if (_old_style != _new_style) {
				...
				if (_indent) {
					...
					_buffer_size += _func_delete_and_insert_buffer(
						...
						_buffer_size,
						2,
						(_bullet_sprite == undefined)
							? "- "
							: "[" + sprite_get_name(_bullet_sprite) + "] "
					);
				} else {
					_indent = true;
					_buffer_size += _func_delete_and_insert_buffer(
						...
						(_bullet_sprite == undefined)
							? "- [indent]"
							: "[" + sprite_get_name(_bullet_sprite) + "] [indent]"
					);
				}
				...
				while (true) {
					var _number_next_value = buffer_peek(
						_buffer,
						_number_peek,
						buffer_u8
					);
		...
		if (_value == ord("*")) {
			...
			(_value == ord("_")) && ((_prev_value <= 0x20) || (_next_value <= 0x20))
		) {
			//Italic only
			__SCRIBBLE_MARKDOWN_TOGGLE_ITALIC;
			...
			if (!_in_link && (_value == ord("["))) {
				...
				if (_is_link) {
					...
					while (true) {
						var _region_next_value = buffer_peek(
							_buffer,
							_region_end,
							buffer_u8
						);
						if (
							(_region_next_value == 0x00)
							|| (_region_next_value == ord(")"))
						) {
							break;
						}
						++_region_end;
