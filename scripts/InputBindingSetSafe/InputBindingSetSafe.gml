...
function InputBindingSetSafe(
	...
	var _collisionArray = InputBindingFindCollisions(
		_forGamepad,
		_binding,
		_verbIndexA,
		_playerIndex,
	...
	if (array_length(_collisionArray) == 0) {
		...
		if ((_verbIndexA != _verbIndexB) || (_alternateA != _alternateB)) {
			__InputTrace(
				"Collision found in (forGamepad=",
				_forGamepad,
				", verb=",
				InputVerbGetExportName(_verbIndexB),
				", alternate=",
				_alternateB,
				")"
			);
			...
			__InputTrace(
				"New binding (",
				_binding,
				") is the same as existing binding (forGamepad=",
				_forGamepad,
				", verb=",
				InputVerbGetExportName(_verbIndexA),
				", alternate=",
				_alternateA,
				")"
			);
