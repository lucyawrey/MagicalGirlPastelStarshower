...
function scribble_typists_add_event(_name, _function) {
	...
	if (variable_struct_exists(__scribble_config_colours(), _name)) {
		__scribble_trace(
			"Warning! Event name \"" + _name + "\" has already been defined as a colour"
		);
		exit;
	}
	...
	if (ds_map_exists(_effects_map, _name)) {
		__scribble_trace(
			"Warning! Event name \"" + _name + "\" has already been defined as an effect"
		);
		exit;
	}
	...
	if (ds_map_exists(_macros_map, _name)) {
		__scribble_trace(
			"Warning! Macro name \"" + _name + "\" has already been defined as a macro"
		);
		exit;
	}
	...
	if (!is_undefined(_old_function)) {
		if (is_numeric(_old_function) && (_old_function < 0)) {
			__scribble_trace(
				"Warning! Overwriting event [" + _name + "] tied to <invalid script>"
			);
		} else {
			__scribble_trace(
				"Warning! Overwriting event ["
					+ _name
					+ "] tied to \""
					+ (is_method(_old_function)
						? string(_old_function)
						: script_get_name(_old_function))
					+ "\""
			);
		}
	}
	...
	if (SCRIBBLE_VERBOSE) {
		__scribble_trace(
			"Tying event ["
				+ _name
				+ "] to \""
				+ (is_method(_function) ? string(_function) : script_get_name(_function))
				+ "\""
		);
	}
}
