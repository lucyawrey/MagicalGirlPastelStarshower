...
function __ChatterboxClassSource(_filename, _buffer, _compile) constructor {
	...
	try {
		...
		__ChatterboxError(
			"\"" + filename + "\" could not be parsed. This source file will be ignored"
		);
		exit;
	}
	...
	repeat (array_length(_nodes_temp_array)) {
		...
		if (!variable_struct_exists(_node_metadata, "title")) {
			...
			var _node = new __ChatterboxClassNode(
				...
				_compile,
				_buffer,
				_node_temp_struct.buffer_start,
				_node_temp_struct.buffer_end
			);
...
function __ChatterboxParseChatterScript(_buffer) {
	...
	if (buffer_get_size(_buffer) >= 4) {
		...
		if ((_first_u32 & 0x0000FFFF) == 0x0000FEFF) {
			buffer_seek(_buffer, buffer_seek_start, 2);
		}
		if ((_first_u32 & 0x0000FFFF) == 0x0000FFFE) {
			buffer_seek(_buffer, buffer_seek_start, 2);
		}
		if ((_first_u32 & 0x00FFFFFF) == 0x00BFBBEF) {
	...
	repeat (buffer_get_size(_buffer) - buffer_tell(_buffer)) {
		...
		var _entered_comment =
			!_in_comment
			&& (_byte == 47)
			&& (buffer_peek(_buffer, buffer_tell(_buffer), buffer_u8) == 47);
			...
			if (!_in_comment && (buffer_tell(_buffer) > _string_start + 1)) {
				...
				if (_line_is_file_tag) {
					if (__CHATTERBOX_DEBUG_LOADER) {
						__ChatterboxTrace("Found file tag \"", _string_trimmed, "\"");
					}
					if (CHATTERBOX_ESCAPE_FILE_TAGS) {
						_string_trimmed = __ChatterboxUnescapeString(_string_trimmed);
					}
					...
					if (_string_trimmed == "---") {
						//Separator between node header and node body
						if (_in_body) {
							__ChatterboxTrace(
								"Warning! \"---\" found outside of node header definition"
							);
						} else {
							_in_body = true;
							_body_start = buffer_tell(_buffer);
						}
					} else if (_string_trimmed == "===") {
						//Node terminator
						if (!_in_body) {
							__ChatterboxTrace(
								"Warning! \"===\" found outside of node body definition"
							);
						} else {
							if (__CHATTERBOX_DEBUG_LOADER) {
								...
								__ChatterboxTrace(
									"Creating node \"",
									__ChatterboxStringLimit(_string, 100),
									"\"    ",
									_node_metadata
								);
							}
						...
						//Treat everything in the header as key:value pairs
						var _colon_pos = string_pos(":", _string_trimmed);

						if (_colon_pos == 0) {
							__ChatterboxTrace(
								"Warning! String found in a node header tag was not a key:value pair (",
								_string,
								")"
							);
						} else {
							var _key = string_copy(_string_trimmed, 1, _colon_pos - 1);
							var _value = string_copy(
								_string_trimmed,
								_colon_pos + 1,
								string_length(_string_trimmed) - _colon_pos
							);
							...
							if (variable_struct_exists(_node_metadata, _key)) {
								__ChatterboxTrace(
									"Warning! Duplicate node metadata found \"",
									_key,
									"\""
								);
							}
