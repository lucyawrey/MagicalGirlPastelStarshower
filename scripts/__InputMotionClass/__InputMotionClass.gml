...
function __InputMotionClass(_device) constructor {
	...
	if (INPUT_ON_SWITCH || INPUT_ON_PS4 || INPUT_ON_PS5) {
		...
		__hasData =
			is_numeric(_steamHandle)
			&& is_struct(steam_input_get_motion_data(_steamHandle));
	} else {
		__hasData = false;
	}
	...
	static __Update = function() {
		...
		switch (os_type) {
			case os_switch:
				...
				accelerationX =
					_signX * switch_controller_acceleration(__device, _axisX, _sensor);
				accelerationY = -switch_controller_acceleration(
					__device,
					switch_controller_axis_z,
					_sensor
				);
				accelerationZ =
					_signZ * switch_controller_acceleration(__device, _axisZ, _sensor);

				angularVelocityX =
					_signX
					* degtorad(
						switch_controller_angular_velocity(__device, _axisX, _sensor) / 5
					)
					* 570.6;
				angularVelocityY =
					-degtorad(
				...
				angularVelocityZ =
					_signZ
					* degtorad(
						switch_controller_angular_velocity(__device, _axisZ, _sensor) / 5
					)
					* 570.6;
				break;
			...
			case os_linux:
				...
				if (is_numeric(_steamHandle)) {
					...
					var _angleRoll = __InputMotionQuatToPitch(
						_deltaX,
						_deltaY,
						_deltaZ,
						_deltaW
					);
					var _anglePitch = -__InputMotionQuatToRoll(
						_deltaX,
						_deltaY,
						_deltaZ,
						_deltaW
					);
					var _angleYaw = -__InputMotionQuatToYaw(
						_deltaX,
						_deltaY,
						_deltaZ,
						_deltaW
