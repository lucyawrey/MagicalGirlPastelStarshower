...
if (InputPressed(INPUT_VERB.UP)) {
	...
	if (selected_index < 0) selected_index = _options_length - 1;
}

if (InputPressed(INPUT_VERB.ACCEPT)) {
	if (array_length(selected_menu.options) == 0) return;
	...
	switch (_option.action) {
		...
		case MENU_ACTION.SWITCH_MENU:
			if (_option.menu != undefined) switch_to_menu(_option.menu);
			return;
